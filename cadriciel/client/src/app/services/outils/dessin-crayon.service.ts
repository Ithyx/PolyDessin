...
export class DessinCrayonService implements InterfaceOutils {
  ...
  sourisEnfoncee(souris: MouseEvent) {
    ...
    this.stockageSVG.setSVGEnCours(
      `<path fill="transparent" stroke="${this.couleur.couleurs[Portee.Principale]}" stroke-linecap="round" stroke-width="`
  ...
  sourisCliquee(souris: MouseEvent) {
    if (this.peutCliquer) {
      if (this.outils.outilActif.parametres[0].valeur) {
        ...
        + this.outils.outilActif.parametres[0].valeur / 2 + `" fill="${this.couleur.couleurs[Portee.Principale]}"/>`;
        this.stockageSVG.ajouterSVG(SVG);
        console.log(this.couleur.couleurs[Portee.Principale]);
      }
      this.traitEnCours = false;
